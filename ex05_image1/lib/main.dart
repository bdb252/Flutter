    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          /* 해당 배율(3.0x)에 이미지가 있으므로 해당 배율의 분홍 이미지가
          출력된다. 만약 이미지가 하나밖에 없다면 노란색 이미지가 출력될 것이다.
          이처럼 현재 디바이스의 화면 배율에 맞는 동일한 이름의 이미지가 있다면
          자동으로 선택해서 출력하게 된다. */
          Image.asset('assets/images/300x300b.png'),
          /* 해당 배율의 이미지라도 크기를 지정하면 지정한 크기가 적용된다.
          width 속성으로 이미지가 표현되는 상자의 크기를 지정하고, fit 속성으로
          가로/세로의 채움 여부를 결정할 수 있다. */
          Image.asset(
