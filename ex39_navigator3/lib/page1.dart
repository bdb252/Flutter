...
class Page1 extends StatefulWidget {
  ...
  String data = '';
  // 교안에 있는 생성자 코드(구버전에서 사용)
  // Page1({Key? key, required this.data}) : super(key: key);
  
  /*
  새로운 현재 버전의 생정자 코드로 required로 선언하면 필수사항임을
          ...
          children: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                /* Page2를 스택에 추가한다. 즉 전환이 아니라 페이지를 추가한다.
                최초 push했을때는 문제가 없지만, pop을 통해 Page2가 종료될때 Null에 대한
                오류가 생기므로 Nullable로 처리해야 한다. */ 
                ...
                /* 앞에서 Page2를 push해서 스택에 추가하면 아래 코드는 바로 실행되지 않고
                await 하게 된다. 즉 Page2가 제거될때까지 대기했다가 닫히는 순간 아래 코드가 실행된다.
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                await Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => Page3(data: '1페이지에서 보냄(1->3)'),
                  ),
                ).then((value) {
